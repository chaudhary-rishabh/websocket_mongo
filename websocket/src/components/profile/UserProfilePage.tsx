...
function InfoRow({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: string }) {
  return (
    <div className="flex items-center gap-3 px-5 py-3.5">
      <Icon className="w-[18px] h-[18px] text-[#9B7653] flex-shrink-0" />
      <div className="min-w-0">
        <p className="text-[11px] text-[#9A8474] uppercase tracking-wide">{label}</p>
        <p className="text-sm font-medium text-[#2A1F14] truncate">{value}</p>
      </div>
    </div>
  )
}
      ...
      <div className="max-w-md mx-auto px-4 py-6 flex flex-col gap-3">
        ...
        >
          ...
          <div className="px-5 pb-5 flex flex-col items-center -mt-10">
            ...
            <h2 className="mt-3 text-base font-bold text-[#2A1F14]">{user.name}</h2>

            {user.role && (
              <span className="mt-1 text-[11px] font-semibold uppercase tracking-wide text-[#7C5C3E] bg-[#EDE4D6] px-2.5 py-0.5 rounded-full">
                {user.role}
              </span>
            )}
            ...
            <div className="mt-5 flex gap-3 w-full">
              {dmConversation ? (
                <Link
                  ...
                  className="flex-1 flex items-center justify-center gap-2 bg-[#7C5C3E] hover:bg-[#9B7653] text-white text-sm font-semibold py-2.5 rounded-xl transition-colors duration-200"
                >
        ...
        {sharedGroups.length > 0 && (
          ...
          >
            ...
            {sharedGroups.map((group, i) => (
              <div key={group.id}>
                {i > 0 && <div className="h-px bg-[#E0D5C5] mx-5" />}
                <Link
                  href={`/chat/${group.id}`}
                  className="flex items-center gap-3 px-5 py-3.5 hover:bg-[#EDE4D6] transition-colors"
                >
                  <Avatar initials={group.initials} name={group.name} id={group.id} size="sm" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#2A1F14] truncate">{group.name}</p>
                    <p className={cn('text-xs text-[#9A8474] flex items-center gap-1 mt-0.5')}>
